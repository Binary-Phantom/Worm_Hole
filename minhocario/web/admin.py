...
def get_image(filename):
    return send_from_directory('..minhocario/media', filename)
    ...
    if current_user.id == 1:
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                db.session.commit()
                flash(f'{product_name} adicionado com sucesso')
                print('Produto Adicionado')
                return render_template('add_shop_items.html', form=form)
            except Exception as e:
                print(e)
                flash('Produto não adicionado')
    ...
    if current_user.id == 1:
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                db.session.commit()
                flash(f'{product_name} Atualizado com sucesso')
                print('Produto atualizado')
                return redirect('/shop-items')
            except Exception as e:
                print('Produto não atualizado', e)
                flash('Item não atualizado')
...
def delete_item(item_id):
    if current_user.id == 1:
        try:
            ...
            print('Item não removido', e)
            flash('Item não removido')
        return redirect('/shop-items')
...
def update_order(order_id):
    if current_user.id == 1:
        ...
        if form.validate_on_submit():
            ...
            try:
                db.session.commit()
                flash(f'Pedido {order_id} atualizado com sucesso')
                return redirect('/view-orders')
            except Exception as e:
                print(e)
                flash(f'Pedido {order_id} não atualizado')
                return redirect('/view-orders')
